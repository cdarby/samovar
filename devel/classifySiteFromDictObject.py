...
#featureDict = pymp.shared.dict(cPickle.load(open(args.featuredict,'rb')))
print(len(featureDict))
clf = joblib.load(args.clf) #each process loads clf
samfile = pysam.AlignmentFile(args.bam, "rb") #each process opens file - may depend on chrom if file is split
...
with pymp.Parallel(NPAR) as pymp_context:
    for region_index in pymp_context.range(len(R)):
        ...
        for read in samfile.fetch(chrom, start, end,multiple_iterators=True): 
            ...
            (mapq,strand,clip,ind,XH,XL,XV,XR,XB) = readFeatures(read,featureDict)#Shared)
