...
fprfromthreshold <- function(p,A) { #predicted positives; all
  return(length(p$status[p$status==0])/(length(A$status[A$status==0])))
}
...
X <- read.delim(header=F,"/Users/charlotte/Documents/lab/samovar/testdata_DO_NOT_COMMIT/giab/depth_mosaicsim/unphasedfeatures/20000_unphased.txt",sep="\t",col.names=c("status","score"))
...
auc(getaxis(X,A,xaxisfunction),getaxis(X,A,yaxisfunction))
