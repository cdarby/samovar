...
def readFeatures(read):
    ...
    return (clip,clipLen,ind,HP,pair,strands) 

def siteFeatures(S):
    (bases, bqs, readpos, clip, ind, HP, pair, strands, clipLen) = (S.bases, S.bqs, S.readpos, S.clip, S.ind, S.HP, S.pair, S.strands, S.clipLen)
    ...
    if Cavgpos < MIN_AVG_POS_FROM_END or (SOFT_CLIP and Cclip == nC) or (GOOD_PAIR and Cpair == nC) or (STRAND and (Cstrand == 0 or Cstrand == nC)): #or Cavgind > MAX_AVG_INDELS 
    ...
    if (SOFT_CLIP and Nclip == nN) or (GOOD_PAIR and Npair == nN) or (STRAND and (Nstrand == 0 or Nstrand == nN)): #Navgind > MAX_AVG_INDELS or 
    ...
    if Mavgpos < MIN_AVG_POS_FROM_END or (SOFT_CLIP and Mclip == nminor) or (GOOD_PAIR and Mpair == nminor) or (STRAND and (Mstrand == 0 or Mstrand == nminor)): #Mavgind > MAX_AVG_INDELS or 
    ...
    if Javgpos < MIN_AVG_POS_FROM_END or (SOFT_CLIP and Jclip == nmajor) or (GOOD_PAIR and Jpair == nmajor) or (STRAND and (Jstrand == 0 or Jstrand == nmajor)): #Javgind > MAX_AVG_INDELS or 
        ...
        with pymp.Parallel(NPAR,if_=(NPAR > 1)) as pymp_context:
            for region_index in pymp_context.xrange(len(R)): #pymp.xrange returns an iterator and corresponds to dynamic scheduling.
                ...
                for read in samfile.fetch(chrom, start, end, multiple_iterators=True): 
                    ...
                    (clip,clipLen,ind,HP,pair,strands) = readFeatures(read)
